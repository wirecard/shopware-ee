...
describe('Credit Card Non 3-D Secure Authorization test', () => {
    before(async () => {
        driver = await getDriver('credit card non3ds authorization');
    });

    const paymentLabel = config.payments.creditCard.label;
    const formFields = config.payments.creditCard.fields;

    it('should check the credit card non3ds authorization payment process', async () => {
